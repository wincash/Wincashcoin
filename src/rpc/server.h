...
/**
 * WINCASHCOIN RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZwccSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzwccseed(const UniValue& params, bool fHelp);
extern UniValue getzwccseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzwcc(const UniValue& params, bool fHelp);
extern UniValue dzwccstate(const UniValue& params, bool fHelp);
