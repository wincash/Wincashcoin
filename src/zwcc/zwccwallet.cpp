...
CzWCCWallet::CzWCCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zwcc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZWCCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZWCCSeed_deprecated()) {
                    LogPrintf("%s: Updated zWCC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zwcc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zwcc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzWCCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZWCCCount(nCountLastUsed);
    else if (!walletdb.ReadZWCCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzWCCWallet::Lock()
{
    seedMaster = 0;
}

void CzWCCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzWCCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZWCC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzWCCWallet::LoadMintPoolFromDB()
{
...
void CzWCCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzWCCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzWCCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zwccTracker->HasPubcoinHash(pMint.first)) {
...
bool CzWCCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZWCC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zwccTracker which also adds to database
    pwalletMain->zwccTracker->Add(dMint, true);
...
void CzWCCWallet::SeedToZWCC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzWCCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzWCCWallet::GenerateDeterministicZWCC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzWCCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzWCCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzWCCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
