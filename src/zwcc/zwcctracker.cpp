// Copyright (c) 2018-2019 The WINCASHCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zwcc/deterministicmint.h>
#include "zwcctracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzWCCTracker::CzWCCTracker(std::string strWalletFile)
{
...
CzWCCTracker::~CzWCCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzWCCTracker::Init()
{
...
bool CzWCCTracker::Archive(CMintMeta& meta)
{
...
bool CzWCCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzWCCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzWCCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzWCCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzWCCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzWCCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzWCCTracker::GetSerialHashes()
{
...
CAmount CzWCCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzWCCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzWCCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzWCCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzWCCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzWCCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzWCCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzWCCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzWCCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzWCCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzWCCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzWCCWallet* zWCCWallet)
{
    bool iszWCCWalletInitialized = (NULL != zWCCWallet);
    CMintMeta meta;
    ...
    if (! iszWCCWalletInitialized)
        zWCCWallet = new CzWCCWallet(strWalletFile);
    meta.isSeedCorrect = zWCCWallet->CheckSeed(dMint);
    if (! iszWCCWalletInitialized)
        delete zWCCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzWCCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzWCCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzWCCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzWCCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzWCCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzWCCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzWCCWallet* zWCCWallet = new CzWCCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zWCCWallet);
        }
        delete zWCCWallet;
        LogPrint("zero", "%s: added %d dzwcc from DB\n", __func__, listDeterministicDB.size());
    }
