...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "wincashcoin" is a composite category enabling all WINCASHCOIN-related debug output
            if (ptrCategory->count(std::string("wincashcoin"))) {
...
#else
    const char* pszModule = "wincashcoin";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\WINCASHCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\WINCASHCOIN
// Mac: ~/Library/Application Support/WINCASHCOIN
// Unix: ~/.wincashcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WINCASHCOIN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "wincashcoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty wincashcoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override wincashcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "wincashcoind.pid"));
