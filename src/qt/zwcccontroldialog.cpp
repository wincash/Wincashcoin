...
#include "ui_zwcccontroldialog.h"
...
std::set<std::string> ZWccControlDialog::setSelectedMints;
std::set<CMintMeta> ZWccControlDialog::setMints;

bool CZWccControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZWccControlDialog::COLUMN_DENOMINATION || column == ZWccControlDialog::COLUMN_VERSION || column == ZWccControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZWccControlDialog::ZWccControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZWccControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZWccControlDialog::~ZWccControlDialog()
{
    delete ui;
}

void ZWccControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWccControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZWccControlWidgetItem* itemDenom(new CZWccControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZWccControlWidgetItem *itemMint = new CZWccControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zwccTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zwccTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zWCC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zWCC seed used to mint this zWCC is not the same as currently hold in the wallet";
            else
...
void ZWccControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWccControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZWcc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWccControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZWccControlDialog::GetSelectedMints()
{
...
void ZWccControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
