...
public:
    ...
    bool WriteZWCCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZWCCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZWCCSeed_deprecated(uint256& seed);
    bool EraseZWCCSeed();
    bool EraseZWCCSeed_deprecated();

    bool WriteZWCCCount(const uint32_t& nCount);
    bool ReadZWCCCount(uint32_t& nCount);
